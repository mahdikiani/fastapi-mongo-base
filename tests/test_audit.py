...
from fastapi_mongo_base.audit.context import (
    AuditActor,
    bind_audit_actor,
    is_audit_enabled,
    reset_audit_actor,
...
from fastapi_mongo_base.audit.diff import compute_changes
...
from fastapi_mongo_base.db.mongo import (
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("widget_db")
async def test_create_update_delete_emit_audit_rows() -> None:
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("widget_db")
async def test_no_audit_when_disabled() -> None:
...
async def test_sql_audit_table_created_only_when_enabled() -> None:
    ...
    from fastapi_mongo_base.audit.sql import deactivate_sql_audit_log
    from fastapi_mongo_base.core.db import close_sql, init_sql
...
async def test_sql_crud_emits_delete_action() -> None:
    ...
    try:
        ...
        activate_sql_audit_log()
        with patch(
            "fastapi_mongo_base.sql.models.async_session",
            session_factory,
        ):
            token = bind_audit_actor(
                AuditActor(
