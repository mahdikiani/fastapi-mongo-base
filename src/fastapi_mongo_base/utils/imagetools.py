"""Image processing utilities using PIL/Pillow."""
...
def add_watermark_to_image(
    ...
    """
    Add a watermark image to a background image at a specified position.
...
def split_image(
    image: Image.Image,
    sections: tuple[int, int] = (2, 2),
    **kwargs: object,
) -> list[Image.Image]:
    """Split an image into a grid of sections."""
    parts = []
...
def is_aspect_ratio_valid(
    ...
    *,
    ...
    """Check if image aspect ratio is within tolerance of target ratio."""
...
def has_white_border(image: Image.Image, *, ratio: float = 0.9) -> bool:
    ...
    for i in range(width):
        pixels.extend((
            image.getpixel((i, 0)),
            image.getpixel((i, height - 1)),
        ))
    for j in range(height):
        pixels.extend((image.getpixel((0, j)), image.getpixel((width - 1, j))))
...
def square_pad_white_pixels(image: Image.Image) -> Image.Image:
    """Pad image to square with white pixels."""
    width, height = image.size
...
def convert_image(
    ...
    **kwargs: object,
    ...
    """
    Convert an image to the specified format while handling transparency.
    ...
    Args:
        ...
        image_format: Target format (JPEG, PNG, WEBP, BMP, GIF).
        bg_color: Background color for formats that do not support
            transparency.
    ...
    if supports_transparency:
        return (
            image.convert("RGBA") if has_transparency else image.convert("RGB")
        )
...
def _parse_image_from_content(content: BytesIO) -> Image.Image | None:
    ...
    content.seek(0)
    try:
        image = Image.open(BytesIO(content.getvalue()))
        image.load()
    except Exception:
        return None
    else:
...
async def get_image_metadata(
    ...
    **kwargs: object,
    ...
    """
    Fetch an image URL and return its metadata.

    Args:
        url: URL of the image.
        use_range: Whether to try a range request to download only
            a header part.
        max_bytes: Maximum number of bytes to request in a range request.
        fallback: If range request does not provide enough data,
            download the full file.
        with_exif: Whether to include EXIF data in metadata.
        **kwargs: Additional keyword arguments.

    Returns:
        Dictionary with image metadata (e.g., width, height, file_type,
        and content_type).
    ...
    if url.startswith("data:image"):
        ...
        return {
            ...
            "content_type": url.split(":")[1].split(";", maxsplit=1)[0],
            "mode": image.mode,
        }

    try:
        response, content = await _fetch_image_data(
            url,
            use_range,
            max_bytes,
            **kwargs,
        ...
        if use_range and fallback:
            ...
            if image:
                return _extract_metadata_from_image(image, response, with_exif)
            msg = "Could not determine image dimensions after full download"
            raise ValueError(
                msg,
            )
        msg = "Could not determine image dimensions with range request data"
        raise ValueError(
            msg,
        )
...
async def download_image(
    ...
    **kwargs: object,
    ...
    """
    Fetch, resize, remove metadata, and compress an image.

    Processes an image to fit the specified constraints.
    """
...
async def download_image_base64(
    ...
    **kwargs: object,
    ...
    """Download and process image, returning base64 encoded string."""
